...
if __name__ == '__main__':
  ...
  def particleFilterInfer(ripl, ct):
    ripl.infer({"transitions":1, "kernel":"pgibbs", "scope":"state", "block":"ordered"})
...
def run(arg):
  ...
  history = model.runFromConditional(50, runs=2, verbose=True, name=name, infer=inference)
  filename = randomword()
...
pool = Pool(6)
id = str(randomint())
pool.map(run, [(id + "_hmm_defaultMH", None), 
               (id + "_hmm_particleFilterInfer",particleFilterInfer),
               (id + "_hmm_reasonableInfer",reasonableInfer),
               ])
