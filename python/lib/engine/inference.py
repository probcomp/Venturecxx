...
class Infer(object):
  ...
  def likelihood_at(self, scope, block):
    return self.engine.trace_handler.map('likelihood_at', scope.getSymbol(), block.getSymbol())
  def posterior_at(self, scope, block):
    return self.engine.trace_handler.map('posterior_at', scope.getSymbol(), block.getSymbol())
