...
class Infer(object):
  ...
  def likelihood_at(self, scope, block):
    return self.engine.model.traces.map('likelihood_at', scope.getSymbol(), block.getSymbol())
  def posterior_at(self, scope, block):
    return self.engine.model.traces.map('posterior_at', scope.getSymbol(), block.getSymbol())
