...
def expToDict(exp):
  ...
  if tag == "mh":
    ...
    useDeltaKernels = exp[4] if len(exp) > 4 else False
    deltaKernelArgs = exp[5] if len(exp) > 5 else None
    updateValue = exp[6] if len(exp) > 6 else None
    ...
    useDeltaKernels = exp[7] if len(exp) > 7 else False
    deltaKernelArgs = exp[8] if len(exp) > 8 else None
    updateValue = exp[9] if len(exp) > 9 else None
