...
function Plotter(doc, svg, ns) {
	...
	this.xSVGToGraph = function(x) {
        return x * (this.maxX - this.minX) / this.svg.width.baseVal.value + this.minX;
    }

    this.xGraphToSVG = function(x) {
        return (x - this.minX) * this.svg.width.baseVal.value / (this.maxX - this.minX);
    }
