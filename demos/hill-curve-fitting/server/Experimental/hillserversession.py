from flask import Flask, json, request, make_response, current_app, session
...
class VentureServer(Flask):
	...
	def add_ripl(self):
		...
		self.ripls[session['ripl_id']] = ripl
		ripl.label = 0
		ripl.point_labels = {}

	def initialize(self):
		ripl = self.ripls[session['ripl_id']]
		ripl.clear()
...
def ventureCall():
	if 'ripl_id' not in session:
		print 'adding ripl'
		session['ripl_id'] = urandom(32)
		app.add_ripl()	
	try:
		j = request.get_json()
		calls = j['calls']
