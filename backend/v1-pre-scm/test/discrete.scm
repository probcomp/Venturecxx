...
(define-test (assessable-coupled-beta-bernoulli-transition-operator)
  ...
  (let ()
    ...
    (define (true-post-infer-dist init)
      (freqs-bind '((0 . 1/2) (1 . 1/2)) (lambda (prop-index)
      (let ((prop-value-dist
             (if (= prop-index 0)
                 '((#t . 1/2) (#f . 1/2))
                 (if (car init)
                     '((#t . 2/3) (#f . 1/3))
                     '((#t . 1/3) (#f . 2/3))))))
        (freqs-bind prop-value-dist (lambda (prop-value)
        (let ((accept-prob
               (if (= prop-index 1)
          ...
          (freqs-bind `((#t . ,accept-prob) (#f . ,(- 1 accept-prob))) (lambda (accept)
          (freqs-return
           (if accept
               (if (= prop-index 0)
    ...
    (define answer
      (freqs-normalize
       (freqs-bind initial-dist (lambda (init)
       (freqs-bind (true-post-infer-dist init) (lambda (post)
      ...
      (let ((prop-value-dist
             (if (= prop-index 0)
                 ...
                 (if (car init)
                     ...
                     '((#t . 1/3) (#f . 2/3))))))
        (freqs-bind prop-value-dist (lambda (prop-value)
        (let* ((assessment-weight
                (cond ((equal? init '(#t . #t)) 3/4)
                      ((equal? init '(#t . #f)) 1/2)
                      ((equal? init '(#f . #t)) 1/2)
                      ((equal? init '(#f . #f)) 1/4)))
               ;; Note assessment against the full state generated by
               ;; the initial draw; the proper thing to assess against
               ;; would have been only the state computed up to the
               ;; point at which this choice is made.
          ...
          ; (pp `(model ,init ,prop-index ,prop-value ,assessment-weight ,accept-prob))
          (freqs-bind `((#t . ,accept-prob) (#f . ,(- 1 accept-prob))) (lambda (accept)
          (freqs-return
           (if accept
               (if (= prop-index 0)
    ...
    (define assess-bug-answer
      (freqs-normalize
       (freqs-bind initial-dist (lambda (init)
       (freqs-bind (assess-bug-post-infer-dist init) (lambda (post)
       (freqs-return (list init post))))))
       data<))
    #;
