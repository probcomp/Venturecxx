...
python -m venture.knight.driver -e '{ t = get_current_trace(); trace_set(t, 5); trace_get(t) }' # (0, 5)
...
python -m venture.knight.driver -f backend/knight/prelude.vnts -f backend/knight/normal-normal.vnts \
  ...
  subt = t2[0, "app"];
  list(trace_get(subt["x"]), trace_get(subt))
...
python -m venture.knight.driver -f backend/knight/prelude.vnts -f backend/knight/normal-normal.vnts \
  ...
  t1 = get_current_trace();
  trace_set(subtrace(subtrace(t1, 0), "app"), 5);
  t2 = get_current_trace();
  res = regenerate(model, [], t1, t2);
  subt = t2[0, "app"];
  list(first(res), trace_get(subt["x"]), trace_get(subt))
...
python -m venture.knight.driver -f backend/knight/prelude.vnts -f backend/knight/normal-normal.vnts \
  ...
  t3 = get_current_trace();
  t4 = get_current_trace();
  trace_set(t4[3, "app", 0, "app", 0, "app", 6, "def", "app"], 7);
  res = regenerate(regenerator_of(normal_normal), [[0, 1, 1], t1, t2], t3, t4);
  list(res, trace_get(t2["x"]), t4)
