...
class BlockScaffoldIndexer(object):
  ...
  def sampleIndex(self,trace):
    if self.block == "one": return constructScaffold(trace,[trace.getNodesInBlock(self.scope,trace.sampleBlock(self.scope))],useDeltaKernels=self.useDeltaKernels,deltaKernelArgs=self.deltaKernelArgs,updateValue=self.updateValue)
    elif self.block == "all": return constructScaffold(trace,[trace.getAllNodesInScope(self.scope)],useDeltaKernels=self.useDeltaKernels,deltaKernelArgs=self.deltaKernelArgs,updateValue=self.updateValue)
