...
{
  double weight = 0;
  shared_ptr<PSP> psp = trace->getOperatorPSP(node);
	      ...
	      shared_ptr<map<Node*,Gradient> > gradients)
{
  ...
  for (size_t i = 0; i < requests.first.size(); ++i)
  {
    ...
    if (!trace->containsSPFamilyAt(requestNode,esr.id))
    {
      ...
      if (shouldRestore)
      {
        esrParent = db->getESRParent(trace->getOperatorSP(node),esr.id);
    ...
    Node * esrParent = trace->getOperatorSPFamily(requestNode,esr.id);
