...
def main(data_source_, epsilon_, N_):
  ...
  output_dir = "data/output/jointdplr"
  ...
  if data_source == "synthetic":
    ...
    data_file = input_dir + '/circle_data'
    N, D, X, y, Ntst, Xtst, ytst = loadData(data_file)
    print "N:", N, "Ntst:", Ntst, "D:", D
  elif data_source == "four_cluster":
    data_file = input_dir + '/four_cluster_data2'
    N, D, X, y, Ntst, Xtst, ytst = loadData(data_file)
    print "N:", N, "Ntst:", Ntst, "D:", D
  else:
    assert False
  ...
  tag = "_".join(["jointdplr", data_source, tag_N, tag_austerity])
  if isPyPy:
    tag += '_pypy'
  ...
  prog = """
  [assume D {D}]
  [assume mu_w (repeat 0 (+ D 1))]
  [assume sigma_w (repeat {sigma_w!r} (+ D 1))]
  [assume m0 (repeat 0 D)]
  [assume k0 {k0}]
  [assume v0 {v0}]
  [assume S0 (diag_matrix (repeat v0 D))]
  ...
  for n in xrange(N):
    if (n + 1) % round(N / 10) == 0:
      print "Processing %d/%d observations." % (n + 1, N)
    v.observe('(y %d (vector %s))' % (n, ' '.join(repr(x) for x in X[n])), y[n])
  t_obs = time.clock() - tic
  ...
  for i in xrange(Nsamples):
    ...
    for z in zCountsTable:
      ...
      ws.append(va.getArray(vv.NumberType()))
    rst['ws'].append(ws)
