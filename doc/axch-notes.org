...
- Idea:
  - [next] Have the transcript machine feed strings into a console
    instance rather than a ripl
...
- Live bugs:
  ...
  + plot('aaoeu', aoeu) (single quotes) causes the prompt to enter an
    infinite "more input please" loop.
  + infer posterior in the console is different from infer posterior
  ...
  - sample returning the same value repeatedly is a mental model
    sticking point
...
- More recent live bugs
  - documentation of wishart, inv_wishart
  - venture[script] > inv_wishart(id_matrix(2), 1)
    *** evaluation: expected square matrix
    (inv_wishart (id_matrix 2) 1)
    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    Caused by
    expected square matrix
  - [Later] Stopping multiline query interpretation on blank line will
    break paste of e.g. definitions that are large enough to have gaps.
...
- Fix egregious introduced discrepancies
  ...
  - There appears to be considerable code duplication between the
    church' and VS parsers (e.g., the Trie class?)
  - I can probably flush the old parser class too
  - Can I get rid of the dependency on pyparsing?
  - examples/hmm.vnt is wrong (the current CP solution fixes whatever
    the bug was).
- Crosscat
- Delete things
  - extra parsed directives and support code
  - operator substitution
  - does the core sivm still have _modify ?
  - can I flush the SIVM's observation reporting cache?
- System cleaning
