...
def _testLoadModel(riplThunk):
    ...
    model = unit.Analytics(v,queryExps=queryExps)
    ...
    model = unit.Analytics(v,assumes=assumes,observes=observes,queryExps=queryExps)
    attributesMatch()
...
def _testCompareSampleDicts(sameDistribution):
    ...
    cReport = unit.compareSampleDicts(dicts,('',''),plot=False)
...
def _testGewekeTest():
    params = unit.generateMRiplParams(no_ripls=(2,3), backends=('puma','lite'),
