...
def testSliceBasic1():
  ...
  predictions = collectSamples(ripl,"pid",infer="(slice default one 0.5 100 %s)" % default_num_transitions_per_sample())
...
def testSliceStudentT2():
  ...
  ripl.observe("(normal a 1.0)", 3.0)
  ripl.predict("(normal a 1.0)", label="pid")
  predictions = collectSamples(ripl,"pid",infer="(slice default one 0.5 100 %s)" % (default_num_transitions_per_sample() * 4))
