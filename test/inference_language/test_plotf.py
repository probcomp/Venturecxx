...
def testPlotfToFile1():
  ...
  ripl.infer('(cycle ((mh default one 10) (plotf_to_file test1 h0 x)) 10)')
  testfile = 'test1.png'
...
def testPlotfToFile2():
  ...
  ripl = get_ripl()
  ripl.assume('x', '(normal 0 1)')
  ripl.infer('(cycle ((mh default one 10) (plotf_to_file (test1 test2) (h0 lcd0d) x)) 10)')
...
def checkPlotfToFileBadArgs(basenames, specs):
  ...
  print infer
  with assert_raises(VentureValueError):
    ripl.infer(infer)
