...
def testDynamicScope1():
  ...
  assert_equal(len(ripl.sivm.core_sivm.engine.getDistinguishedTrace().getNodesInBlock(0,0)),1)
...
def testDynamicScope2():
  ...
  assert_equal(len(ripl.sivm.core_sivm.engine.getDistinguishedTrace().getNodesInBlock(0,0)),2)
...
def testDynamicScope3():
  ...
  assert_equal(len(ripl.sivm.core_sivm.engine.getDistinguishedTrace().getNodesInBlock(0,0)),3)
...
def testDynamicScope4():
  ...
  assert_equal(len(ripl.sivm.core_sivm.engine.getDistinguishedTrace().getNodesInBlock(0,0)),4)
...
def testDynamicScope5():
  ...
  ripl.assume("f", "(mem (lambda () (normal x 1.0)))")  
  ripl.predict("(scope_include 0 0 (normal (+ (f) (normal (f) 1) (normal 0 1)) 1))")
  assert_equal(len(ripl.sivm.core_sivm.engine.getDistinguishedTrace().getNodesInBlock(0,0)),5)
