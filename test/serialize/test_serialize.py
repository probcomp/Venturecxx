...
def _test_serialize_program(v, label, action):
    ...
    engine.modelz.traces = engine.modelz.create_handler([trace2])
    r2 = collectStateSequence(v, label, infer=infer)

    engine.modelz.traces = engine.modelz.create_handler([trace1])
...
def check_foreign_sp(mode):
    ...
    engine.modelz.traces = engine.modelz.create_handler([restored])
